...
namespace newRBS.ViewModels
{
    ...
    {
        public AsyncObservableCollection<NodeViewModel> Items { get; set; }
    }
    ...
    public class NodeViewModel : ViewModelBase
    {
        ...
        public AsyncObservableCollection<NodeViewModel> Children { get; set; }
    }
    ...
    public class SpectraFilterViewModel : INotifyPropertyChanged
    {
        ...
        private NodeViewModel _selectedFilter;
        public NodeViewModel selectedFilter
        {
            get
            ...
            set
            {
                ...
                spectraListViewModel.ChangeFilter(value);
            }
        }
        ...
        private void TreeViewItemSelectedChangedCallBack(ViewModelUtils.TreeViewHelper.DependencyPropertyEventArgs e)
        {
            ...
            {
                NodeViewModel temp = (NodeViewModel)e.DependencyPropertyChangedEventArgs.NewValue;
        ...
        public SpectraFilterViewModel()
        {
            ...
            filterTree.Items = new AsyncObservableCollection<NodeViewModel>();
            selectedFilter = new NodeViewModel() { Name = "All", Type = "All" };
        ...
        private void _TestButtonClick()
        {
            ...
            spectraListViewModel.spectraList[0].Channel = 99;
        }
        ...
        private void FillFilterList(string filterType)
        {
            ...
            {
                foreach (NodeViewModel n in filterTree.Items)
            ...
            {
                case "Date":
                    filterTree.Items.Add(new NodeViewModel() { Name = "All", Type = "All" });
                    filterTree.Items.Add(new NodeViewModel() { Name = "Today", Type = "Date", SubType = "Today" });
                    filterTree.Items.Add(new NodeViewModel() { Name = "This Week", Type = "Date", SubType = "ThisWeek" });
                    filterTree.Items.Add(new NodeViewModel() { Name = "This Month", Type = "Date", SubType = "ThisMonth" });
                    filterTree.Items.Add(new NodeViewModel() { Name = "This Year", Type = "Date", SubType = "ThisYear" });

                    List<int> allYears = dataSpectra.GetAllYears();
                    foreach (int Year in allYears)
                    {
                        NodeViewModel newYearNode = new NodeViewModel() { Name = Year.ToString(), Type = "Date", SubType = "Year", year = Year };

                        List<int> allMonths = dataSpectra.GetAllMonths(Year);
                        if (allMonths.Count > 0)
                        {
                            newYearNode.Children = new AsyncObservableCollection<NodeViewModel>();
                            foreach (int Month in allMonths)
                            {
                                NodeViewModel newMonthNode = new NodeViewModel() { Name = Month.ToString("D2"), Type = "Date", SubType = "Month", year = Year, month = Month };

                                List<int> allDays = dataSpectra.GetAllDays(Year, Month);
                                if (allDays.Count > 0)
                                {
                                    newMonthNode.Children = new AsyncObservableCollection<NodeViewModel>();
                                    foreach (int Day in allDays)
                                    {
                                        NodeViewModel newDayNode = new NodeViewModel() { Name = Day.ToString("D2"), Type = "Date", SubType = "Day", year = Year, month = Month, day = Day };
                ...
                case "Channel":
                    ...
                    {
                        Console.WriteLine(Channel);
                        filterTree.Items.Add(new NodeViewModel() { Name = Channel.ToString(), Type = "Channel", channel = Channel });
                    }
                    break;
                default:
